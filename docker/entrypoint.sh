...
# Skip TCP wait when using Cloud SQL Unix socket; nc no sirve con rutas.
if printf '%s' "$DB_HOST" | grep -qE '^/cloudsql/|/'; then
    printf 'DB_HOST (%s) parece socket Unix, no se hace wait con nc\n' "$DB_HOST"
else
